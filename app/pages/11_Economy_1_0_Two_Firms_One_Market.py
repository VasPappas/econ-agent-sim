"""Two independent firms in one compact, linked-period market workspace."""

from math import fsum

import streamlit as st

from econ_agent_sim.competition_chat_view import render_competition_chat
from econ_agent_sim.competition_comparison import compare_competition_runs
from econ_agent_sim.competition_experiment_view import (
...
from econ_agent_sim.results_1_0_component import render_competition_results
...
def capture():
    ...
    for index, household in enumerate(st.session_state.cg_households):
        for field in HOUSEHOLD_FIELDS:
            key = f"cg_household_{field}_{index}"
            household[field] = float(st.session_state.get(key, household[field]))
    for index, firm in enumerate(st.session_state.cg_firms):
        for field in FIRM_FIELDS:
            key = f"cg_firm_{field}_{index}"
...
def remember_expander(name):
    st.session_state.cg_expanded[name] = st.session_state[f"cg_open_{name}"]


def setup_expander(label, name, *, expanded=False):
    key = f"cg_open_{name}"
    st.session_state.setdefault(
        key, st.session_state.cg_expanded.get(name, expanded)
    )
...
def resize():
    capture()
    households = st.session_state.cg_households
    count = st.session_state.cg_count
    defaults = default_households(count)
    st.session_state.cg_households = [
        households[index] if index < len(households) else defaults[index]
        for index in range(count)
    ]
    for index in range(count, 20):
        for field in HOUSEHOLD_FIELDS:
            st.session_state.pop(f"cg_household_{field}_{index}", None)
        name = f"household_{index}"
        st.session_state.pop(f"cg_open_{name}", None)
        st.session_state.cg_expanded.pop(name, None)


def reset():
    generation = st.session_state.cg_generation + 1
    baseline = st.session_state.cg_baseline
    for key in list(st.session_state):
        if key.startswith("cg_"):
            del st.session_state[key]
    st.session_state.cg_generation = generation
    st.session_state.cg_baseline = baseline
...
def select_period():
    st.session_state.cg_period_focus = st.session_state.cg_selected


def remember_report_scope():
    st.session_state.cg_saved_scope = st.session_state.cg_report_scope


def submitted_settings():
    return (
        tuple(Household(**item) for item in st.session_state.cg_households),
        tuple(Firm(**item) for item in st.session_state.cg_firms),
    )
...
def start():
    ...
    try:
        ...
        candidate = advance_competition_period(households, firms)
    except (ValueError, ArithmeticError, AssertionError) as error:
        st.session_state.cg_error = f"Could not start this setup: {error}"
        return
    st.session_state.cg_history = [candidate]
    st.session_state.cg_submitted = (households, firms)
    if st.session_state.cg_selected_firm not in {firm.id for firm in firms}:
        st.session_state.cg_selected_firm = firms[0].id
    st.session_state.cg_generation += 1
    st.session_state.cg_selected = 1
    st.session_state.cg_period_focus = 1
    st.session_state.cg_error = None
    st.session_state.cg_next_view = "Results"
    if name := st.session_state.pop("cg_copy_name", None):
        st.session_state.cg_experiment_name = name
        st.session_state.cg_name_input = name


def next_period(count=1):
    history = st.session_state.cg_history
    if not history or len(history) >= MAX_PERIODS:
        return
    households, firms = st.session_state.cg_submitted
    pending = []
    previous = history[-1]
    try:
        for _ in range(min(count, MAX_PERIODS - len(history))):
            previous = advance_competition_period(households, firms, previous=previous)
            pending.append(previous)
    except (ValueError, ArithmeticError, AssertionError) as error:
        st.session_state.cg_error = f"Could not advance this economy: {error}"
        return
    st.session_state.cg_history = [*history, *pending]
    st.session_state.cg_selected = len(history) + len(pending)
    st.session_state.cg_period_focus = len(history) + len(pending)
    st.session_state.cg_error = None


def compact_input(owner, field, label, *, minimum, maximum, step, index=None):
    suffix = f"_{index}" if index is not None else ""
    with st.container(key=f"cg_compact_{owner}_{field}{suffix}"):
        ...
        with input_column:
            st.number_input(
                ...
                key=f"cg_{owner}_{field}{suffix}", on_change=capture,
                label_visibility="collapsed",
            )


st.set_page_config(
    page_title="Tiny Economy — Two Firms, One Market", layout="centered",
...
for key, value in {
    ...
    "saved_scope": st.session_state.get("cg_report_scope", "This period"),
}.items():
    st.session_state.setdefault(f"cg_{key}", value)
initialize_experiments()

apply_workspace_style()
st.markdown(
    """<style>
    .st-key-cg_mobile_nav [data-testid="stHorizontalBlock"],
    .st-key-cg_period_controls [data-testid="stHorizontalBlock"],
    .st-key-cg_experiment_actions [data-testid="stHorizontalBlock"],
    .st-key-cg_baseline_actions [data-testid="stHorizontalBlock"],
    [class*="st-key-cg_compact_"] [data-testid="stHorizontalBlock"] {
        flex-direction: row !important; flex-wrap: nowrap !important; gap: .5rem;
    }
    .st-key-cg_mobile_nav [data-testid="stColumn"],
    .st-key-cg_period_controls [data-testid="stColumn"],
    .st-key-cg_experiment_actions [data-testid="stColumn"],
    .st-key-cg_baseline_actions [data-testid="stColumn"],
    [class*="st-key-cg_compact_"] [data-testid="stColumn"] { min-width: 0; }
    .st-key-cg_mobile_nav [data-testid="stButtonGroup"] { width: 100%; }
    .st-key-cg_mobile_nav button {
        min-height: 44px; padding: .4rem .45rem; border-radius: 12px;
        flex: 1; white-space: nowrap;
    }
    .st-key-cg_mobile_nav button p { font-size: .8rem; }
    .st-key-cg_mobile_nav button[aria-checked="true"],
    .st-key-cg_report_scope button[aria-checked="true"] {
        background: #174e44 !important; color: white !important;
        border-color: #174e44 !important;
    }
    .st-key-cg_report_scope button { min-height: 44px; }
    .st-key-cg_experiment_actions button,
    .st-key-cg_baseline_actions button { min-height: 44px; padding: .4rem; }
    .st-key-cg_experiment_actions button p,
    .st-key-cg_baseline_actions button p { font-size: .8rem; }
    [class*="st-key-cg_compact_"] { margin-bottom: -.5rem; }
    [class*="st-key-cg_compact_"] [data-testid="stMarkdownContainer"] p {
        font-size: .85rem; line-height: 1.25; margin: 0;
    }
    [class*="st-key-cg_compact_"] [data-testid="stNumberInput"] button {
        min-width: 44px; min-height: 44px; width: 44px;
    }
    [class*="st-key-cg_compact_"] [data-testid="stNumberInput"] input {
        min-width: 0; padding-left: .5rem; padding-right: .2rem;
    }
    [class*="st-key-cg_open_"] summary { min-height: 48px; }
    [class*="st-key-cg_open_firm_"] details { background: #fffdf5; border-color: #e6d8ad; }
    .st-key-cg_starting_totals {
        border: 1px solid #dce4d7; background: #edf3ea;
        border-radius: 14px; padding: .8rem 1rem;
    }
    .st-key-cg_starting_totals p { margin: 0; }
    @media(max-width: 360px) {
        .st-key-cg_mobile_nav [data-testid="stHorizontalBlock"] { gap: .3rem; }
        .st-key-cg_mobile_nav button { padding-inline: .3rem; }
        .st-key-cg_mobile_nav button p { font-size: .75rem; }
    }
    </style>""", unsafe_allow_html=True,
)
st.caption("TINY ECONOMY · 1.0 · TWO FIRMS, ONE MARKET")
st.page_link("streamlit_app.py", label="← Explore economies")
if target := st.session_state.pop("cg_next_view", None):
    st.session_state.cg_view = target
with st.container(key="cg_mobile_nav"):
    ...
    with view_column:
        view = st.pills(
            ...
            key="cg_view", label_visibility="collapsed", width="stretch",
        )
    with reset_column:
        st.button(
...
if notice := st.session_state.pop("cg_notice", None):
    st.success(notice)
if st.session_state.cg_error:
...
try:
    ...
    draft_settings = None
dirty = bool(history) and draft_settings != st.session_state.cg_submitted

if view == "Set up":
    st.title("Two firms, one market.")
    st.write(
        "Two firms make the same good. They hire from the same households, "
        "receive the same price and pay the same wage."
    )
    st.number_input(
        "Households", min_value=2, max_value=20, step=1,
        key="cg_count", on_change=resize,
    )
    st.caption("Each firm uses its own money and capital. Change either firm to see what happens.")

    for index, firm in enumerate(st.session_state.cg_firms):
        for field in FIRM_FIELDS:
            value = firm[field]
            st.session_state.setdefault(
                f"cg_firm_{field}_{index}",
    ...
    for index, household in enumerate(st.session_state.cg_households):
        for field in HOUSEHOLD_FIELDS:
            st.session_state.setdefault(
                f"cg_household_{field}_{index}", household[field]
            )
    ...
    count = len(st.session_state.cg_households)
    household_money = fsum(item["money"] for item in st.session_state.cg_households)
    firm_money = fsum(item["money"] for item in st.session_state.cg_firms)
    capital = fsum(item["capital"] for item in st.session_state.cg_firms)
    with st.container(key="cg_starting_totals"):
    ...
    st.caption("Reset restores two equal households and two equal firms, with 1 total capital.")
elif not history:
    ...
    st.session_state.setdefault("cg_selected", st.session_state.cg_period_focus)
    selected = st.selectbox(
        "View period", options=list(range(1, len(history) + 1)),
        format_func=lambda number: f"Period {number}", key="cg_selected",
        on_change=select_period, label_visibility="collapsed",
    )
    with st.container(key="cg_period_controls"):
    ...
    st.session_state.setdefault("cg_report_scope", st.session_state.cg_saved_scope)
    report_scope = st.pills(
        "Report range", options=("This period", "Cumulative"), required=True,
        key="cg_report_scope", width="stretch", label_visibility="collapsed",
    ...
    comparison = compare_competition_runs(
        ...
        current_name=st.session_state.cg_experiment_name,
    ...
    if view == "Results":
        ...
        render_competition_chat(
            history[selected - 1], report,
            st.session_state.cg_generation, "cg_chat",
            comparison=comparison,
        )
