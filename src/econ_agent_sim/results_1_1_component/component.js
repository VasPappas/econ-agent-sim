...
export default function render({ data, parentElement, setStateValue }) {
  ...
  const targetSummary = (account, wholeEconomy = false) => {
    ...
    header.append(el('strong', below ? 'target-shortfall' : 'target-met',
      below ? cumulative ? 'Shortfalls occurred' : 'Below target'
        : account.shortfall_x > 0 ? 'Within tolerance' : 'Target met'));
    section.append(header);
    const bar = el('div', 'target-bar');
    bar.setAttribute('role', 'img');
    bar.setAttribute('aria-label', `${percent(coverage, 1)} of consumption targets covered; ${fmt(account.shortfall_x)} X shortfall`);
    const fill = el('span', below ? 'target-fill shortfall' : 'target-fill');
    fill.style.width = `${coverage * 100}%`;
    bar.append(fill);
    section.append(bar);
    section.append(row(cumulative ? 'Total targets' : wholeEconomy ? 'Combined targets' : 'Target this period', `${fmt(account.needed_x)} X`));
    section.append(row(cumulative ? 'Sum of period shortfalls' : 'Shortfall', `${fmt(account.shortfall_x)} X`, below ? 'target-gap' : ''));
    if (cumulative) {
      section.append(el('p', 'note', 'Extra consumption cannot erase an earlier shortfall.'
        + (wholeEconomy ? ' One household’s extra consumption cannot cover another’s shortfall.' : '')));
    } else if (wholeEconomy && below) {
      section.append(el('p', 'note', `${account.households_below_target} of ${report.households.length} households below target. Extra consumption by others does not cover their shortfall.`));
    }
    return section;
  };
  ...
  shell.setAttribute('aria-label', 'Consumption target results');
  shell.append(el('div', 'topline', `${report.label} · ${report.households.length} HOUSEHOLDS · TWO FIRMS`));
  ...
  report.households.forEach((household, index) => {
    ...
    body.append(el('p', 'note', `Consumption target · ${fmt(household.parameters.consumption_target)} X each period. The preference scores stay fixed; shortfalls add urgency without guaranteeing the target.`));
  ...
  download.download = `economy-11-${report.label.toLowerCase().replaceAll(' ', '-').replace('–', '-')}-accounts.csv`;
  ...
  shell.append(el('p', 'boundary', 'Targets apply each period; shortfalls do not become debt or build up extra urgency. Money and capital carry forward. No borrowing or money creation.'));
  root.append(shell);
}
