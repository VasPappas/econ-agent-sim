...
from econ_agent_sim.target_explanations import (
    target_context,
    target_explanations,
)


def render_target_chat(period, report, run_id, view_key, *, comparison=None):
    st.subheader("Make sense of your economy")
    st.caption(f"{report['label']} · Answers use the results you are viewing.")
    answers = target_explanations(period, report)
    ...
    st.divider()
    context = target_context(period, report, run_id, comparison=comparison)
    fingerprint = context_id(context)
    conversations = st.session_state.setdefault("target_conversations", {})
