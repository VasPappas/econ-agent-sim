"""Versioned portable workspaces for the two-firm economy.
...
ENGINE_VERSION = "consumption-targets-1.1.0"
...
class LegacyExperimentError(ExperimentError):
    """A recognized earlier experiment belongs in its original chapter."""

    def __init__(self, message: str, economy: str):
...
class Experiment:
    ...
    def __post_init__(self) -> None:
        ...
        if self.periods:
            ...
            if not isinstance(first, Economy11Period):
                raise ExperimentError("Use completed two-firm periods.")
            _settings(first.households, first.firms)
            firms = first.firms
            for number, period in enumerate(self.periods, 1):
                if not isinstance(period, Economy11Period) or (
...
def _parse_settings(value: object) -> tuple[tuple[Household, ...], tuple[Firm, ...]]:
    ...
    if not isinstance(items, list) or len(items) != 2:
        raise ExperimentError("Economy 1.1 needs exactly two firms.")
    firms = []
    for item in items:
...
def _digest(period: Economy11Period) -> str:
...
def _restore(payload: object) -> Experiment:
    ...
    if payload["run"] is not None:
        ...
        for expected in digests:
            previous = advance_target_period(households, firms, previous)
...
def load_experiment(data: bytes | str) -> ExperimentFile:
    ...
    try:
        ...
        if (
            ...
            and payload.get("engine_version") == "investment-growth-0.9.1"
        ):
            raise LegacyExperimentError(
                "This is an Economy 0.9 experiment. Open it in Investment and Growth "
                "to preserve its original accounts. Your target experiment is unchanged.",
                "0.9",
            )
        if (
            isinstance(payload, dict)
            and payload.get("format") == FORMAT
            and type(payload.get("format_version")) is int
            and payload["format_version"] == 2
            and payload.get("model") == "competition"
            and payload.get("engine_version") == "two-firms-1.0.0"
        ):
            raise LegacyExperimentError(
                "This is an Economy 1.0 experiment. Open it in Two Firms, One Market "
                "to preserve its original results. Your target experiment is unchanged.",
                "1.0",
            )
