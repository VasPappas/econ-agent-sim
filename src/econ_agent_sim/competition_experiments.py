...
ENGINE_VERSION = "two-firms-1.0.0"
...
class LegacyExperimentError(ExperimentError):
    """A recognized investment experiment belongs in its original chapter."""
...
class Experiment:
    ...
    def __post_init__(self) -> None:
        ...
        if self.periods:
            ...
            if not isinstance(first, Economy10Period):
            ...
            for number, period in enumerate(self.periods, 1):
                if not isinstance(period, Economy10Period) or (
    ...
    for item in items:
        ...
        households.append(Household(
            ...
            **{
                ...
                for field in household_fields - {"id", "name", "money"}
            },
        ))
    items = settings["firms"]
    if not isinstance(items, list) or len(items) != 2:
        raise ExperimentError("Economy 1.0 needs exactly two firms.")
    firms = []
    for item in items:
...
def _digest(period: Economy10Period) -> str:
    ...
    try:
        ...
        if (
            ...
            raise LegacyExperimentError(
                ...
                "to preserve its original accounts. Your two-firm experiment is unchanged."
            )
