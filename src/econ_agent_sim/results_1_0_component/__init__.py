"""Phone-sized accounts for two firms and their household owners."""
...
_KEY = "cg_results_component"


def _remember_selected_firm():
    selected = st.session_state.get(_KEY, {}).get("selected_firm")
    submitted = st.session_state.get("cg_submitted")
    allowed = {firm.id for firm in submitted[1]} if submitted else set()
    if selected in allowed:
        st.session_state.cg_selected_firm = selected


def render_competition_results(data):
