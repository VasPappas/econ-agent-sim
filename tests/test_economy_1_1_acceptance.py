"""Independent optimality, accounting and compatibility for flexible targets.
...
import pytest
from test_economy_1_0_acceptance import assert_funded_ledger, close

from econ_agent_sim.economy_1_0 import Household as Household10
...
def periods_for(households, firms, count):
    ...
    for _ in range(count):
        periods.append(advance_target_period(
...
def test_zero_targets_match_original_economics_for_100_linked_periods():
    households = (
    ...
    original_households = tuple(Household10(
        h.id, h.name, h.money, h.consumption_priority, h.money_priority, h.leisure_priority,
    ) for h in households)
    firms = (replace(FIRMS[0], money=.2, productivity=3), replace(FIRMS[1], money=.8))
    old = new = None
    for _ in range(100):
        old = advance_competition_period(original_households, firms, old)
        new = advance_target_period(households, firms, new)
        for key in ("price", "wage", "output", "capital_close", "investment_quantity",
                    "net_operating_profit", "dividends_paid", "equity_close"):
            close(getattr(new, key), getattr(old, key), old.equity_close)
        for field in ("work", "leisure", "consumption", "closing_cash", "dividends"):
